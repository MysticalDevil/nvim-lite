...
│       │   ├── prog.lua         # blink.cmp、conform、lint、neotest、dap
...
| prog | blink.cmp、conform.nvim、nvim-lint、neotest、nvim-dap、rustaceanvim、lazydev |
...
- 全局编辑器映射直接定义在 `lua/devil/core/keymaps.lua`
- 插件触发映射直接写在对应 `specs/*.lua` 的 `keys` 字段
- LSP 与 gitsigns 这类 buffer-local 映射在 attach 回调中注册
- 不再维护集中式 keymap 模板和转换层
